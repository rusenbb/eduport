...
/// Build the tauri-specta `Builder` with every command this binary
/// exposes. Extracted so the TS-binding export test can call it
/// without spinning up the full Tauri app, and so `run()` and the
/// test share one source of truth for the command list.
...
pub fn run() {
    // tauri-specta builder. Drives Tauri's command registration
    // (replacing the old `tauri::generate_handler!`). The TypeScript
    // bindings export runs as a unit test (see
    // `tests::export_typescript_bindings`) so they're regenerated by
    // `cargo test` rather than the GUI bootstrap path.
...
#[cfg(test)]
mod tests {
    /// Generates `frontend/src/lib/bindings.ts` from the command list.
    /// Runs every `cargo test` so the bindings stay in lockstep with
    /// the Rust command surface — if you change a command's
    /// signature, this test regenerates the TS file and (when the
    /// frontend's `svelte-check` next runs) the type system surfaces
    /// any frontend mismatch. The file is committed so production
    /// builds don't depend on running this test.
    #[test]
    fn export_typescript_bindings() {
        // i64 counts (parse_errors, entities, count) fit in JS Number
        // — they're cardinalities, never large enough to exceed
        // Number.MAX_SAFE_INTEGER (2^53 - 1). Map BigInt → number so
        // the bindings stay ergonomic on the frontend side.
        let ts = specta_typescript::Typescript::default()
            .bigint(specta_typescript::BigIntExportBehavior::Number);
        super::build_specta_builder()
            .export(ts, "../../frontend/src/lib/bindings.ts")
